...
// ── Init ───────────────────────────────────────────────────────────────
await Actor.init();
const { companyUrls = [], maxReviewsPerCompany = 100, sortBy = 'latest', filterByStars = 'all', includeCompanyInfo = true, proxyConfig, } = (await Actor.getInput()) ?? {};
...
function extractSlug(input) {
    ...
    if (match)
        return match[1].replace(/-\d{6,}$/, ''); // strip numeric DB IDs like -201712110
...
function buildReviewPageUrl(slug, page) {
    let url;
    if (page === 1) {
    ...
    if (filterByStars !== 'all')
        params.set('starRating', filterByStars);
    const qs = params.toString();
    return qs ? `${url}?${qs}` : url;
}
// ── Extraction (runs inside browser context) ────────────────────────────
async function extractCompanyInfo(page, slug) {
    return page.evaluate((slug) => {
        ...
        const reviewCount = parseInt(countEl?.getAttribute('content') || countEl?.textContent?.trim() || '0') || 0;
...
async function extractReviews(page, slug, companyName) {
    const rawReviews = await page.evaluate((slug) => {
        ...
        const reviewEls = document.querySelectorAll('div[itemscope][itemprop="review"][itemtype*="schema.org/Review"], ' +
            '[itemscope][itemtype*="schema.org/Review"]');
        for (const el of reviewEls) {
            const title = el.querySelector('h2[itemprop="headline"], .review-item-title')?.textContent?.trim() || '';
            const reviewBody = el.querySelector('[itemprop="reviewBody"]')?.textContent?.trim() || '';
            const ratingText = el.querySelector('[itemprop="ratingValue"]')?.textContent?.trim()
                || el.querySelector('.stars-wrap')?.getAttribute('data-active-count') || '0';
            ...
            const authorName = el.querySelector('[itemprop="author"] [itemprop="name"]')?.textContent?.trim()
                || el.querySelector('[itemprop="author"]')?.textContent?.trim() || '';
            const locationParts = [];
            ...
            const helpfulText = el.querySelector('.vote-yes .count')?.textContent?.trim() || '0';
        ...
        if (results.length === 0) {
            ...
            for (const card of cards) {
                ...
                const body = card.querySelector('[class*="body"], [class*="text"], [class*="content"] p')?.textContent?.trim() || '';
                if (!title && !body)
                    continue;
...
const crawler = new PlaywrightCrawler({
    ...
    requestHandler: async ({ request, page, crawler: c }) => {
        ...
        await page.waitForSelector('[itemprop="review"], [class*="review-card"], [class*="review-item"], [class*="complaint-item"]', { timeout: 10000 }).catch(() => {
